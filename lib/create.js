...
async function create(projectName,options){
  const cwd = options.cwd || process.cwd() // 当前文件路径
  // 是否在当前目录
  const inCurrent = projectName === '.'
  const name = inCurrent ? path.relative('../', cwd) : projectName // 当前文件夹名与项目名一样则取当前文件夹名 回到上一级
  const targetDir = path.resolve(cwd, projectName || '.')  // 要创建的目标路径
  const result = validatePackageName(name)
  // 如果所输入的不是合法npm (package.json .name)包名，则退出
